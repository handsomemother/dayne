	...
	$(function() {
		...
		if(/msie/.test(u) && parseInt(v, 10) == 6) { 
			...
			css_string += '' + 
				...
				'.jstree li a { display:inline; border-width:0 !important; padding:0px 0px !important; } ' + 
		...
		__init : function () { 
			...
			this.get_container()
				.delegate("a", "click.jstree", $.proxy(function (event) {
						//TODO zrm 注释掉以下一行，这个会让<a>失去本身的功能。
		...
		_fn : {
				...
				switch(!0) {
					...
					case (!s.data && !!s.ajax) || (!!s.data && !!s.ajax && obj && obj !== -1):
						...
						success_func = function (d, t, x) {
							...
							//zhangruimin 处理预先选中的节点
							if($.isFunction(eval("initSelectedNode"))){
								initSelectedNode(this.get_container());
							}
						};
						s.ajax.context = this;
			...
			_parse_json : function (js, is_callback) {
				...
				if($.isArray(js)) {
					...
					//luohui 第一次加载时判断显示根节点样式。p.core.rootStyle=true根节点不加+-号
					if(!p.core.rootStyle){
						d.prepend("<ins class='jstree-icon'>&#160;</ins>");
					}
					else{
						d.attr("background-color","red");	
					}
					p.core.rootStyle=false;
		...
		__init : function () {
			...
			this.get_container()
				...
				.delegate("a", "click.jstree", $.proxy(function (e) {
						if(!(this.isCheckboxExcluded(this._get_node(e.target)))){
			...
			//zhangruimin isSingle为true时checkScope设置为self,这时配置的checkScope参数失效
			if(s.isSingle){
				s.checkScope="self";
			}
		},
		//zhangruimin 当check状态发生变化时的回调处理函数
		defaults : { 
			check_state_change_handler:false,
			//zhangruimin "all"代表要连动check子孙节点和前辈节点，"self"代表只check本节点
			checkScope: "all",
			//zhangruimin 是单选还是多选，如果是多选，则用checkbox；如果是单选，则用radio
			isSingle : false,
			excludedTypes : false
		},
		__destroy : function () {
		...
		_fn : {
			_prepare_checkboxes : function (obj) {
				...
				obj.each(function () {
					...
					//zhangruimin 直接对ins进行样式控制，以区分radio和checkbox的显示.
					var checkedClass = s.isSingle?"jstree-radio-checked":"jstree-checkbox-checked";
					var notCheckedClass = s.isSingle?"jstree-radio-unchecked":"jstree-checkbox-unchecked";
					var checkboxClass = t.is("li") && t.hasClass("jstree-checked") ? checkedClass:notCheckedClass;
					if(t.is("li") &&!(_this.isCheckboxExcluded(t))){
						t.find("a").not(":has(.jstree-checkbox)").prepend($("<ins class='jstree-checkbox'>&#160;</ins>").addClass(checkboxClass)).parent().not(".jstree-checked, .jstree-unchecked").addClass(c);
					}
				});
				if(obj.is("li")) { this._repair_state(obj); }
				else { obj.find("> ul > li").each(function () { _this._repair_state(this); }); }
			},
			//zhangruimin 改变checkbox的状态，需要根据参数确定改变状态的范围
			change_state : function (obj, state) {
				var _this=this;
				var s = this._get_settings().checkbox;
				obj = this._get_node(obj);
				//zhangruimin 有时取不到obj, //TODO 还不知道怎么回事
				if(!obj){return;}
				state = (state === false || state === true) ? state : obj.hasClass("jstree-checked");
				if(state) { 
					if(s.checkScope==="self"){
						this.checkNodeStateClass(obj, false);
					}else{
						//"all"和"selfAndDesendants"都需要改变自己和子孙节点状态.
						obj.find("li").andSelf().each(function(){
							_this.checkNodeStateClass($(this), false);
						});
					}
				} else { 
					if(s.checkScope==="self"){
						this.checkNodeStateClass(obj, true);
					}else{
						obj.find("li").andSelf().each(function(){
							_this.checkNodeStateClass($(this), true);
						});
					}
				...
				if(s.checkScope==="all"){
					obj.parentsUntil(".jstree", "li").each(function () {
						...
						if(state) {
							...
							if($this.children("ul").children(".jstree-unchecked").length) {
								$this.parentsUntil(".jstree", "li").andSelf().each(function(){
									_this.checkNodeStateClass($this, false);
								});
								return false;
							} else {
				...
				//zhangruimin 看是否需要调用回调函数
				if(s.check_state_change_handler){
					s.check_state_change_handler.call(this,this.get_container(),obj);
				}
				this.__callback(obj);
			},
			//zhangruimin 根据结点类型判断某个节点是否需要加checkbox.
			isCheckboxExcluded: function (obj){
				var s = this._get_settings().checkbox;
				return s.excludedTypes&&($.inArray(this._get_type(obj),s.excludedTypes)>-1);
			},
			//zhangruimin 设置某个节点的li和ins的选中和非选中的class
			checkNodeStateClass: function(obj, state){
				var s = this._get_settings().checkbox;
				var checkedClass = s.isSingle?"jstree-radio-checked":"jstree-checkbox-checked";
				var notCheckedClass = s.isSingle?"jstree-radio-unchecked":"jstree-checkbox-unchecked";
				obj.removeClass("jstree-checked jstree-unchecked").addClass(state?"jstree-checked":"jstree-unchecked");
				obj.children("a").children(".jstree-checkbox").removeClass(checkedClass).removeClass(notCheckedClass);
				obj.children("a").children(".jstree-checkbox").addClass(state?checkedClass:notCheckedClass);
			},
			check_node : function (obj) {
				//zhangruimin 加入对单选的处理
				var s = this._get_settings().checkbox;
				if(s.isSingle){
					this.uncheck_all();
				}
			...
			check_all : function () {
				//zhangruimin 为了能连锁操作，需要暂时将checkScope设为"all"
				var s = this._get_settings().checkbox;
				var _checkScope=s.checkScope;
				s.checkScope = "all";
				var _this = this;
				this.get_container().children("ul").children("li").each(function () {
					_this.check_node(this, false);
				});
				s.checkScope = _checkScope;
			},
			...
			_repair_state : function (obj) {
				//zhangruimin 如果不需要checkbox,直接返回.
				if(this.isCheckboxExcluded(obj)){return;}
				obj = this._get_node(obj);
				if(!obj.length) { return; }
				var a = obj.find("> ul > .jstree-checked").length,
					c = obj.find("> ul > li").length;

				if(a === 0) { this.uncheck_node(obj); }
...
(function ($) {
	...
	$.jstree.plugin("search", {
		...
		_fn : {
			...
			//zhangruimin 需要等所有处理节点打开的ajax请求处理完才在树中查找。非常经典的ajax处理代码。
...
(function ($) {
	$.vakata.context = {
		...
		show	: function (s, t, x, y, d, p) {
			...
			$.vakata.context.cnt.css({ "left" : x, "top" : y })
				.find("li:has(ul)")
...
/*		_fn : {
			show_contextmenu : function (obj, x, y) {
				...
				var nodemenus=settings.contextmenu.nodemenus;
				var s,a,o;
			//	alert(obj.attr("id"));
				$.each(nodemenus, function (i, m) {
				//	if(m.nodeId==obj.context.id){
					if(m.nodeId==obj.attr("id").substr(5,2)){
				        s = this,
					    a = obj.children("a:visible:eq(0)"),
					    o = false;
				        if(s.select_node && tree.data.ui && !tree.is_selected(obj)) {
				        	tree.deselect_all();
				        	tree.select_node(obj, true);
				        }
				        if(s.show_at_node || typeof x === "undefined" || typeof y === "undefined") {
				        	o = a.offset();
				        	x = o.left;
				        	y = o.top + tree.data.core.li_height;
				        }
				        if($.isFunction(s.items)) { 
				        	s.items = s.items.call(tree, obj); 

				        }
				        tree.data.contextmenu = true;
				        $.vakata.context.show(s.items, a, x, y, tree, obj);
				        if(tree.data.themes) { $.vakata.context.cnt.attr("class", "jstree-" + tree.data.themes.theme + "-context"); }
					  }
					}
				);
			}
		...
		_fn : {
			show_contextmenu : function (obj, x, y) {
				...
				//采用预先定义的菜单
				if(s.items==="predefined"){
					var itemOptions=s.itemOptions;
					var menuItemIds=new Array();
					if(obj.attr("menuItems").length>0){
						menuItemIds=obj.attr("menuItems").split('|');
					}
					var menuItems=new Array();
					for( var i=0; i < menuItemIds.length;i++){
						$.each(itemOptions, function (j, val) {
							if (menuItemIds[i] == val.itemId) {
								menuItems[i]=val;
							}
						});
					}
...
(function ($) {
	...
	$(function() {
		var css_string = '.jstree .ui-icon { overflow:visible; } .jstree a { padding:0 0px; }';
		$.vakata.css.add_sheet({ str : css_string });
	});
})(jQuery);
//*/
